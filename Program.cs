...
namespace ISM6225_Assignment_2_Spring_2022
{
    ...
    class Program
    {
        ...
        */
        ...
        Output: "a"
        */
        public static string change_to_lowercase(string s)
        ...
        public static string MostCommonWord(string para, string[] banned)
        {
            try
            {
                ...
                {                     //iterating over the sentence character by charater
                    if (para[i] == ',') continue;                         // condition for eliminating character so that punctuation is ignored
                    if (para[i] == ' ' || para[i] == '.')
                    {            //if this condition meets we got a word in temp variable
                        string new_string = change_to_lowercase(temp);         //function call for changing each word in lowercase
                ...
                {                         //iterating for removing the banned word
                    if (map.ContainsKey(change_to_lowercase(banned[i])))
                    {
                        map[change_to_lowercase(banned[i])] = 0;
                    }
                }
        ...
        public static int Lucky(int[] arr)
        {
            try
            {
                Dictionary<int, int> lucky_no = new Dictionary<int, int>();//creating dictionary for mapping
                foreach (var element in arr)
                {                              //iterating over arr for mapping the value and its frequency
                    if (lucky_no.ContainsKey(element))
                    {
                        lucky_no[element] = lucky_no[element] + 1;
                    }
                    else
                    {
                        lucky_no[element] = 1;
        ...
        */
        ...
        public static string Hint(string secret, string guess)
        {
            try
            {
                ...
                string answer = x + "A" + y + "B";                           //concatenating string for Answer matching
                return answer;
            }
            catch (Exception)
            {
        ...
        */
        ...
        public static List<int> PartitionLabels(string s)
        {
            try
            {
                ...
                for (int i = len - 1; i >= 0; i--)     // loop to iterate through the length for arrayMap
                {
                    if (map[s[i] - 97] == 0)
                    {
                        map[s[i] - 97] = i;
                ...
                int index = 0;             // condition until index is less than length of the input string
                while (index < len)
                {
                    int low = index;
            ...
            try
            {
                ...
                {                                         //iterating over all character
                    ...
                    {       //if closing comes then check in stack and pop for the same
                        if (st.Count <= 0) // if stacklist is empty then returns false
                        {
                            return false;
                        }
                        if (bulls_string10[i] == ')')  // if string is closing braces
                        {
                            if (st.Peek() == '(')
                            {
                                st.Pop();
                        ...
                        if (bulls_string10[i] == '}')  // if string is closing with flower braces
                        {
                            if (st.Peek() == '{')
                            {
                                st.Pop();
                        ...
                        if (bulls_string10[i] == ']') // if string closes with square braces
                        {
                            if (st.Peek() == '[')
                            {
                                st.Pop();
